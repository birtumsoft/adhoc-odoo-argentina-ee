    ...
    * A través de tags: si definimos tags, se buscan impuestos pendientes de
    ...
    def _get_tax_settlement_entry_lines_vals(self, domain=None):
        ...
        for group in grouped_move_lines:
            ...
            if (
                currency := group["account_id"][0]
                and self.env["account.account"].browse(group["account_id"][0]).currency_id
            ):
                ...
                new_vals_line.update({"currency_id": currency.id, "amount_currency": amount_currency})
    ...
    def get_tax_settlement_files_values(self, move_lines):
        ...
        _logger.info("Getting tax settlement tax values for '%s'" % (self.name))
            ...
            dashboard_data[journal.id].update(
                {
                    ...
                    "unsettled_amount": formatLang(
                        self.env, -sum(unsettled_lines.mapped("balance")), currency_obj=currency
                    ),
