...
class InflationAdjustment(models.TransientModel):
    ...
    def get_periods(self, start_date=None, end_date=None):
        ...
        while cur_date < end:
            ...
            index = indexes.filtered(lambda x: x.date >= cur_date and x.date < date_to)
            if not index:
                raise UserError(
                    _(
                        "El asiento de ajuste por inflación no puede ser generado ya que hace falta el indice de ajuste para el periodo %s %s"
                    )
